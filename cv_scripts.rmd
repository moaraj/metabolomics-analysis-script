...
db_search_metabs <- as.character(colnames(final_metab_data[10:30]))
...
HMDB_gene_search <- function(x) {
  ...
  as.character(toString.XMLNode(xmltop[["protein_associations"]][["protein"]][["gene_name"]][["text"]]))
...
metab_gene_list_print <- stringr::str_trim(metab_gene_list_print)
...
library(rtacklayer)
#https://support.bioconductor.org/p/83349/
# R Track Layer Fix if OpenSSL issue coming up
...
gff_file_path <- "/Users/mohasan/Dropbox/Aging BXD Study/D _ Metabolomics/G _ genome ranges script/mm_gff3/Mus_musculus.GRCm38.87.gff3"
gff <- import.gff(gff_file_path, "gff3")
Mouse_genes <- GRanges(gff)

Mouse_genes = geneRanges(Mus.musculus, column="SYMBOL")
Mouse_genes <- sort(Mouse_genes)
Mouse_genes <- keepStandardChromosomes(Mouse_genes)

Mousedata("CRC", package = "biovizBase")
head(sub)

filt.ids <- metab_gene_list_print
Metab_gene_data <- Human_genes[(elementMetadata(Human_genes)[,1] %in%  filt.ids)]


p <- ggbio(trackWidth = 10, buffer = 7, radius = 10) + 
     circle(Metab_gene_data, geom = "rect", color = "steelblue") + #somatic mutation
     circle(hg19sub, geom = "ideo", fill = "gray70") +#Ideogram
     circle(hg19sub, geom = "scale", size = 3) +#Scale
     circle(hg19sub, geom = "text", aes(label = seqnames), vjust = 10, size = 3)#label

p 

q <- ggbio(trackWidth = 10, buffer = 5, radius = 10) + circle(hg19sub, geom = "ideo", fill = "gray70") +
    circle(hg19sub, geom = "scale", size = 2) + circle(hg19sub, geom = "text", 
    aes(label = seqnames), vjust= -2, size = 2) +
    circle(Metab_gene_data, geom = "rect", color = "steelblue")
q
...
names(grl) <- sample(LETTERS, size = length(grl), replace = TRUE) grl
```
